<!-- resources/views/recruteur/offre-edit.blade.php -->

@extends('layouts.app')

@section('title', 'Modifier l\'offre : ' . $offre->titre)
    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('recruteur.offres.show', $offre) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    Voir l'offre
                </a>
            </div>
        </div>
...
@section('content')
<form action="{{ route('recruteur.offres.update', $offre) }}" method="POST">
    @csrf
    @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="6" required
                              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('description', $offre->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="specialite_requise" id="specialite_requise" value="{{ old('specialite_requise', $offre->specialite_requise) }}" required
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <select id="type_contrat" name="type_contrat" required
                            ...
                            <option value="Interim" {{ old('type_contrat', $offre->type_contrat) === 'Interim' ? 'selected' : '' }}>Intérim</option>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    ...
                    <div>
                        ...
                        <input type="number" name="salaire_min" id="salaire_min" min="0" value="{{ old('salaire_min', $offre->salaire_min) }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('salaire_min')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="salaire_max" id="salaire_max" min="0" value="{{ old('salaire_max', $offre->salaire_max) }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('salaire_max')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="date_limite" id="date_limite" value="{{ old('date_limite', $offre->date_limite->format('Y-m-d')) }}" required min="{{ now()->format('Y-m-d') }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('date_limite')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="annees_experience_requises" id="annees_experience_requises" min="0" value="{{ old('annees_experience_requises', $offre->annees_experience_requises) }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="text" name="niveau_etude_requis" id="niveau_etude_requis" value="{{ old('niveau_etude_requis', $offre->niveau_etude_requis) }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                ...
                <div class="flex items-center">
                    <input id="is_urgent" name="is_urgent" type="checkbox" {{ old('is_urgent', $offre->is_urgent) ? 'checked' : '' }}
                ...
                <div class="border-t border-gray-200 pt-6" 
                     x-data="{
                        ...
                        competencesSelectionnees: {{ json_encode($offre->competences->map(function($item) { 
                            return [
                                'id' => $item->id,
                                'nom' => $item->nom,
                                'niveau' => $item->pivot->niveau_requis,
                                'obligatoire' => $item->pivot->est_obligatoire
                            ]; 
                        })) }},
                        searchQuery: '',
                        filteredCompetences: [],
                        ...
                        init() {
                            this.filteredCompetences = this.competences.filter(c => 
                                !this.competencesSelectionnees.some(sc => sc.id === c.id)
                            );
                        },
                                ...
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-md">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <select :name="'competences[' + index + '][niveau]'" x-model="competence.niveau"
                                                class="block w-32 border border-gray-300 rounded-md shadow-sm py-1 px-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                        ...
                                        <div class="flex items-center">
                                            <input :id="'obligatoire_' + index" :name="'competences[' + index + '][obligatoire]'" type="checkbox" x-model="competence.obligatoire"
                                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                            <label :for="'obligatoire_' + index" class="ml-2 block text-xs text-gray-700">
                                                Obligatoire
                                            </label>
                                        </div>
                                        <button type="button" @click="competencesSelectionnees.splice(index, 1); searchCompetences()" class="text-red-600 hover:text-red-500">
